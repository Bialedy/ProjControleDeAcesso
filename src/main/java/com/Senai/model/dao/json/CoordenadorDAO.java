...
public class CoordenadorDAO {
    ...
    private final Gson gson = new Gso
    ...
    private List<Coordenador> carregar(){
        try (FileReader reader = new FileReader(caminho)) {
            Type listType = new TypeToken<List<Coordenador>>() {
            }.getType();
    ...
    public void inserir (Coordenador adm){
    int novoId = coordenadores.steam().mapToInt(Coordenador::getId).max().orElse(0)+1;
    coord.setId(novoId);
